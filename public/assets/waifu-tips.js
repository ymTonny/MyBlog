...
function initWidget(waifuPath, apiPath) {
	...
	function showHitokoto() {
		...
		else $.getJSON("https://api.gushi.ci/all.json", function(result) {
				var text = `这句古诗出自 <span style="color:#0099cc;">『${result.origin}』</span>，是 <span style="color:#0099cc;">${result.author}</span> 所作,属于<span style="color:#0099cc;">${result.category}</span>`;
	...
	function loadModel(modelId, modelTexturesId) {
		...
		loadlive2d("live2d", `${apiURL}/get/?id=${modelId}-${modelTexturesId}`);
	}
